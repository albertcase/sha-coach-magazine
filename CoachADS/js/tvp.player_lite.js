...
if(!!tvp.$.userAgent.ie){try{actObj=new ActiveXObject(QQLive.config.PROGID_QQLIVE_INSTALLER);if(typeof actObj.getVersion!="undefined"){vOcx=actObj.GetVersionByClsid(QQLiveSetup.config.OCX_CLSID);}}catch(err){}}else if(tvp.$.userAgent.isNotIESupport()){var plugs=navigator.plugins,plug;if(!tvp.$.isUndefined(plugs.namedItem)){plug=plugs.namedItem("鑵捐瑙嗛");}
if(!plug){for(var i=0,len=plugs.length;i<len;i++){if(plugs[i]&&plugs[i].name=="鑵捐瑙嗛"||plugs[i].filename=="npQQLive.dll"){plug=plugs[i];break;}}}
...
return;var str=this.getFlashHTML();el.innerHTML=str;flashobj=tvp.$.browser.ie?document.getElementById(this.playerid):document.embeds[this.playerid];$me.onwrite(curVideo.getVid());if(document.domain==""&&document.location.href.substr(0,5)=="file:"&&!!window.console&&(typeof window.console.warn=="function")){window.console.warn("鏈〉闈㈠寘鍚吘璁棰戠粺涓€鎾斁鍣紝濡傛灉鎮ㄥ笇鏈涜皟鐢ㄧ粺涓€鎾斁鍣ˋPI鎺ュ彛锛岃浣跨敤http褰㈠紡鎵撳紑锛屾湰鍦板弻鍑籬tml鏂囦欢鍙兘浼氬鑷碅PI澶辨晥");}};this.getPlayer=function(){return flashobj;}
this.play=function(video){if(!flashobj){throw new Error("鏈壘鍒拌棰戞挱鏀惧櫒瀵硅薄锛岃纭flash鎾斁鍣ㄦ槸鍚﹀瓨鍦�");}
...
if(!videoObj){throw new Error("鏈壘鍒拌棰戞挱鏀惧櫒瀵硅薄锛岃纭<video>鏍囩鏄惁瀛樺湪");}
if(tvp.$.isUndefined(video)){videoObj.play();return;}
if(!video instanceof tvp.VideoInfo){throw new Error("浼犲叆鐨勫璞′笉鏄痶vp.VideoInfo鐨勫疄渚�");}
...
tvp.Player.prototype=new tvp.BasePlayer();tvp.Player.instance=[];/*  |xGv00|708aa7ff5d90a5da5666936cdd49cf7f */